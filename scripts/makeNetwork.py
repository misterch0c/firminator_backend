...
echo "#!/bin/bash">> $WORK_DIR/delete.sh
sudo chmod +x  $WORK_DIR/delete.sh


echo "killall ${QEMU}" >> $WORK_DIR/delete.sh


echo "echo "Deleting route..."" >> $WORK_DIR/delete.sh
echo "sudo route del -host ${GUESTIP} gw ${GUESTIP} ${HOSTNETDEV}" >> $WORK_DIR/delete.sh


echo "echo "Bringing down TAP device..."" >> $WORK_DIR/delete.sh
echo "sudo ifconfig ${TAPDEV} down" >> $WORK_DIR/delete.sh

echo "if [ ${HASVLAN} -ne 0 ]" >> $WORK_DIR/delete.sh
echo "then" >> $WORK_DIR/delete.sh
    echo "echo "Removing VLAN..."" >> $WORK_DIR/delete.sh
    echo "sudo vconfig rem ${HOSTNETDEV}" >> $WORK_DIR/delete.sh
echo "fi" >> $WORK_DIR/delete.sh

echo "echo -n "Deleting TAP device ${TAPDEV}... "" >> $WORK_DIR/delete.sh
echo "sudo tunctl -d ${TAPDEV}" >> $WORK_DIR/delete.sh

echo "echo "Done!"">> $WORK_DIR/delete.sh
